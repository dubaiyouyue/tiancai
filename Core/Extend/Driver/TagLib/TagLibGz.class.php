 ...
 */
if (defined('APP_NAME') != 'GZphp' && !defined("GZPHP"))
    exit("Access Denied");
/*
 *
 * @select wei add by 20140715 新增一个select标签扩展。使用方法：
 * <GZ:select sql="select * from table order by id desc" limit="10" page="1" id="r"></GZ:select>
   <GZ:select table="article" limit="10" id="r"></GZ:select>
 */
class TagLibGz extends TagLib {
    ...
    protected $tags = array(
        ...
        'banner' => array('attr' => 'bannerid,pos,key,id', 'close' => 0),
        ...
        'select' => array('attr' => 'table,where,order,limit,id,page,sql,field,key,mod,debug', 'level' => 3)
    );
    ...
    public function _banner($attr, $content) {
        ...
        $pos = !empty($tag['pos']) ? $tag['pos'] : '';
        $mod = isset($tag['mod']) ? $tag['mod'] : '2';
        $bannerid = !empty($tag['bannerid']) ? $tag['bannerid'] : '';

        if ($bannerid && !is_numeric($bannerid)) {
            if (substr($bannerid, 0, 2) == 'T[') {
                $T = $this->tpl->get('T');
                preg_match_all("/T\[(.*)\]$/", $bannerid, $arr);
                $bannerid = $T[$arr[1][0]];
            } else {
                $bannerid = $this->tpl->get($bannerid);
            }
        }
    ...
    public function _catpos($attr) {
        ...
        if (is_numeric($tag['catid'])) {
            ...
            $parsestr .= '$parsestr[] = \'<a href="\'.str_ireplace(\'/index.php\',\'\',$Categorys[$cid][\'url\']).\'">\'.$Categorys[$cid][\'catname\'].\'</a>\';?>';
