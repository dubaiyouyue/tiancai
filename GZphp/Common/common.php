...
function sendmail($tomail,$subject,$body,$config=''){
		...
		//$mail->From ="wei2l99@foxmail.com";
		//$mail->FromName = "gzphp企业建站系统";
		$mail->SetFrom($config['mail_from'], $config['site_name']);     //发送者邮箱
		$mail->AddAddress($tomail); //可同时发多个
		//$mail->AddReplyTo('wei2l99@foxmail.com', 'gzphp'); //回复到这个邮箱
...
function  sql_split($sql,$tablepre) {
	...
	if($tablepre != "gzphp_") $sql = str_replace("gzphp_", $tablepre, $sql);
...
function get_two($id){
	do{
		...
		$arrparentid = $info['arrparentid'];
		if($arrparentid=='0'){ return $info['id'];}
		$count = count(explode(',',$arrparentid));
		$id = $info['parentid'];
	}while($count!='2');
	return $info['id'];
}
...
function get_user($id){
	return M('user')->where("id='$id' and status='1'")->getfield('realname');
}

function get_modulename($id='0',$modulename='article'){
	return M($modulename)->where("id='$id' and status='1'")->getfield('title');
}
