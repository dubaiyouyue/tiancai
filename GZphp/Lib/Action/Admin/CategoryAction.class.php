...
class CategoryAction extends AdminbaseAction {
        ...
        if ($this->categorys) {
            foreach ($this->categorys as $r) {
                if ($r['module'] == 'Page') {
                    ...
                    if ($r['parentid'] != "0" and $r['child'] == "0") {//wzw add by 20140321 修复有子导航下父导航还能在列表页中有“添加内容”的功能按钮。
                        $r['str_manage'] = '<a href="?g=Admin&m=' . $r['module'] . '&a=add&catid=' . $r['id'] . '">' . L('add_content') . '</a> | ';
                    }
                }
                if ($_SESSION['groupid'] !== "1") {
                    //后台管理员不是userid==1
                    if ($r['parentid'] == "0" and ($r['child'] == "1" or $r['arrchildid'] == $r['id'])) {
                        $r['str_manage'] .='一级栏目不能操作';
                    } else {
    ...
    public function _before_add() {
        ...
        $gzphp_auth = authcode('1-1-0-1-jpeg,jpg,png,gif-3-0', 'ENCODE', $gzphp_auth_key);
    ...
    public function insert() {
        ...
        $_POST['catname'] = safe_replace($_POST['catname']); //过滤空格 wzw add by 20140321
        $_POST['catdir'] = safe_replace($_POST['catdir']); //过滤空格 wzw add by 20140321
    ...
    public function delete() {
        ...
        $this->repair();
        savecache('Category');
        $this->assign('jumpUrl', U('Admin/Category/index', array('menuid' => $_GET['menuid'])));
        $this->success(L('do_success'));
    }
