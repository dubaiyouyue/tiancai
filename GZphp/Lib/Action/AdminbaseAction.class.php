...
class AdminbaseAction extends Action
{
		...
		if(APP_LANG){
			...
			if($_GET['l']){
				if($this->Lang[$_GET['l']]['id']){
					$_SESSION['GZ_lang'] = $_GET['l'];
					$_SESSION['GZ_langid'] = $this->Lang[$_GET['l']]['id'];
				}else{
					$this->error ( L ( 'NO_LANG' ) );
				}
			}elseif(!$_SESSION['GZ_lang'] || !$_SESSION['GZ_langid']){
				$_SESSION['GZ_lang'] = $this->sysConfig['DEFAULT_LANG'];
				$_SESSION['GZ_langid'] = $this->Lang[$this->sysConfig['DEFAULT_LANG']]['id'];
			}
			define('LANG_NAME',$_SESSION['GZ_lang']);
			define('LANG_ID',$_SESSION['GZ_langid']);
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'', './GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
			L(include LANG_PATH.$this->sysConfig['DEFAULT_LANG'].'/common.php');
			$T = F('config_'.$this->sysConfig['DEFAULT_LANG'],'', './GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}
		$this->assign('T',$T);
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,'./GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,'./GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			$r=$this->buildHtml($filename,$dir,'./GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index($sitemap)
    {
		...
		if(empty($sitemap)){
			...
			$this->assign('bcid',0);
		$r=$this->buildHtml('index','./'.$lang,'./GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHtml('sitemap','./'.$lang,'./GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }
    ...
    public function ajax_edit() {
        ...
        if ($field == 'daren' && $field == 0) {
            // $mod->where(array($pk=>$id))->setField('daren_type', '0');
        }
        $this->ajaxReturn(1);
    }
     ...
     * @param string $dialog
     */
    protected function ajaxReturns($status = 1, $msg = '', $data = '', $dialog = '') {
