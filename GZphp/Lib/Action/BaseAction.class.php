...
class BaseAction extends Action
{
    ...
    public function index($catid='',$module='')
    {
		...
		if($catid=='169'){
				...
				$page = new Page ( $new_count, $listRows );
				$page->urlrule = geturl($cat,'');
				$pages = $page->show();
				
				$new_list = $User->where($new_where)->order('listorder desc,createtime desc,id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
				$pages=str_ireplace('/index.php','',$pages);
		...
		if($module=='Guestbook'){
			...
			if($count){
				...
				$field =  $field ? $field : '*';
				$list = $this->dao->field($field)->where($where)->order('listorder desc,posid desc,createtime desc,id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
				$pages=str_ireplace('/index.php','',$pages);
				$this->assign('pages',$pages);
