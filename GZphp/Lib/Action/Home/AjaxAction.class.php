...
class AjaxAction extends BaseAction {
        ...
        if ($do == 'save') {
            ...
            if ($userid) {
                ...
                if ($id) {
                    ...
                    if ($ir) {
                        echo json_encode(array('error' => '收货信息已经存在！'));
                        exit;
                    }
                    $id = $model->add($_POST);
                }
    ...
    public function pages() {
        ...
        $where['tablemodule'] = intval($_POST['mid']);
        $where['tableid'] = intval($_POST['tid']);
        $where['status']=1;
        $total = M('Guestbook')->where($where)->count(); //总记录数
        ...
        $list = M('Guestbook')->field('*')->where($where)->order('id desc')->limit("$startPage,$pageSize")->select();
        foreach ($list as $k => $v) {
            $page['list'][$k] = $v;
            $page['list'][$k]['date'] = toDate($v['createtime'], "Y-m-d");
            $page['list'][$k]['time'] = toDate($v['createtime'], "H:i:s");
        }
